  ...
  const RoutesPage = () => {
    ...
    const calculateTimings = (startTime, haltTime, stops) => {
      ...
      for (let i = 0; i < stops.length; i++) {
        let timeString = `${hours.toString().padStart(2, "0")}:${minutes.toString().padStart(2, "0")} AM`;
          ...
          <ul>
            {routes.length > 0 &&
              routes.map((route, index) => {
                ...
                return (
                  <li key={index} className="mt-4 border p-4 rounded bg-gray-50 shadow-md">
                    ...
                    {route.buses.map((bus, idx) => (
                      <div key={idx} className="ml-4">
                        ...
                        {/* Show More Button */}
                        <button
                          onClick={() => setExpandedRouteIndex(index === expandedRouteIndex ? null : index)}
                          className="mt-2 bg-blue-500 text-white p-1 rounded text-sm"
                        >
                          {expandedRouteIndex === index ? "Show Less" : "Show More"}
                        </button>
