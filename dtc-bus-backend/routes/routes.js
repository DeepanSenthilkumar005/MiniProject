...
router.post("/add-route", async (req, res) => {
  ...
  if (
    ...
    !Array.isArray(haltTimes)
  ) {
    return res
      .status(400)
      .json({ message: "All fields including halt times are required." });
  }
  ...
  if (stops.length !== timings.length || stops.length !== haltTimes.length) {
    return res
      .status(400)
      .json({
        message: "Each stop must have a corresponding timing and halt value.",
      });
  }
  ...
  try {
    ...
    const validRoutes = routes
      .map((route) => {
        ...
        const [minIndex, maxIndex] = [startIndex, destinationIndex].sort((a, b) => a - b);
        const totalTime = route.timings.slice(minIndex, maxIndex).reduce((sum, time) => sum + time, 0);
        const haltTime = route.haltTimes.slice(minIndex, maxIndex).reduce((sum, halt) => sum + halt, 0);
...
router.get("/stops", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching stops", error: error.message });
  }
});
  ...
  try {
    ...
    if (validDestinations.length === 0) {
      return res
        ...
        .json({
          message:
            "No valid destinations found for the selected start location.",
        });
    }
